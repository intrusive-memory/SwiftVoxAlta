...
struct ComponentRegistrationTests {
  ...
  func allSevenVariantsAreRegistered() {
    ...
    #expect(
      ttsComponents.count >= 7,
      "Expected at least 7 qwen3-tts components, found \(ttsComponents.count)")
  }
  ...
  func componentIDsMatchEnumCases() {
    ...
    for repo in Qwen3TTSModelRepo.allCases {
      ...
      if let descriptor {
        #expect(
          descriptor.id == repo.componentId,
  ...
  func huggingFaceRepoMatchesRawValue() {
    ...
    for repo in Qwen3TTSModelRepo.allCases {
      ...
      #expect(
        descriptor.huggingFaceRepo == repo.rawValue,
  ...
  func minimumMemoryBytesIsPositiveForAll() {
    ...
    for repo in Qwen3TTSModelRepo.allCases {
      ...
      #expect(
        descriptor.minimumMemoryBytes > 0,
  ...
  func componentDescriptorsDeclareRequiredFiles() {
    ...
    for repo in Qwen3TTSModelRepo.allCases {
      ...
      for expectedFile in expectedFiles {
        #expect(
          filePaths.contains(expectedFile),
