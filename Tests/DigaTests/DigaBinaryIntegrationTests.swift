...
struct DigaBinaryIntegrationTests {
  ...
  private static func findBinaryPath() throws -> String {
    ...
    guard FileManager.default.fileExists(atPath: binaryPath) else {
      throw TestError.binaryNotFound(
        "diga binary not found at \(binaryPath). " + "Run 'make install' to build the binary."
      )
    }
  ...
  private func validateNotSilence(audioFile: AVAudioFile) throws {
    ...
    print(
      "  Audio analysis: RMS=\(String(format: "%.4f", rms)), "
        + "Peak=\(String(format: "%.4f", peakAmplitude)), " + "Frames=\(buffer.frameLength)")
