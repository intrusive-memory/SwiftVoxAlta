...
public actor VoxAltaVoiceCache {
  ...
  public func store(id: String, data: Data, gender: String?) {
    ...
    FileHandle.standardError.write(
      Data(
        "[VoxAltaVoiceCache] 📥 Storing voice '\(id)' (data hash: \(dataHash), size: \(data.count) bytes)\n"
          .utf8))
  ...
  public func get(id: String) -> CachedVoice? {
    ...
    if result != nil {
      ...
      FileHandle.standardError.write(
        Data("[VoxAltaVoiceCache] 🔍 Retrieved voice '\(id)' (data hash: \(dataHash))\n".utf8))
    } else {
      let cachedIds = voices.keys.sorted().joined(separator: ", ")
      FileHandle.standardError.write(
        Data(
          "[VoxAltaVoiceCache] ❌ Voice '\(id)' NOT FOUND in cache. Available: [\(cachedIds)]\n".utf8
        ))
    }
    return result
  }
