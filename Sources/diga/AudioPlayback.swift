...
enum WAVHeaderParser: Sendable {
  ...
  static func parse(_ data: Data) throws -> WAVHeader {
    ...
    let audioFormat = data.withUnsafeBytes {
      $0.load(fromByteOffset: 20, as: UInt16.self).littleEndian
    }
    guard audioFormat == 1 else {
      throw AudioPlaybackError.invalidWAVData("Not PCM format (audioFormat=\(audioFormat)).")
    }

    let numChannels = data.withUnsafeBytes {
      $0.load(fromByteOffset: 22, as: UInt16.self).littleEndian
    }
    let sampleRate = data.withUnsafeBytes {
      $0.load(fromByteOffset: 24, as: UInt32.self).littleEndian
    }
    let bitsPerSample = data.withUnsafeBytes {
      $0.load(fromByteOffset: 34, as: UInt16.self).littleEndian
    }
    ...
    let dataSize = data.withUnsafeBytes {
      $0.load(fromByteOffset: 40, as: UInt32.self).littleEndian
    }
    ...
    for await wavData in chunks {
      ...
      if !engineStarted {
        ...
        if let existing = connectedFormat,
          existing.sampleRate != format.sampleRate || existing.channelCount != format.channelCount
        {
          throw AudioPlaybackError.unsupportedFormat(
