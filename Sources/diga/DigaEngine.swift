...
enum DigaEngineError: Error, LocalizedError, Sendable {
  ...
  var errorDescription: String? {
    ...
    case .voiceNotFound(let name):
      return
        "Voice '\(name)' not found. Use --voices to list available voices, or --import-vox to import a .vox file."
...
actor DigaEngine {
  ...
  func synthesize(text: String, voiceName: String? = nil, instruct: String? = nil) async throws
    -> Data
  {
    // Resolve voice.
  ...
  func synthesizeFromVox(text: String, voxPath: String, instruct: String? = nil) async throws
    -> Data
  {
    ...
    do {
      ...
      throw DigaEngineError.synthesisFailed(
        "Failed to read .vox file: \(error.localizedDescription)")
    }
    ...
    if FileManager.default.fileExists(atPath: modelSpecificPromptFile.path) {
      ...
      if FileManager.default.fileExists(atPath: voxFile.path) {
        do {
          ...
          FileHandle.standardError.write(
            Data(
              "Warning: could not update .vox file with clone prompt: \(error.localizedDescription)\n"
                .utf8)
          )
        }
      }
      ...
      do {
        ...
        if let audio = sourceAudio {
          ...
          do {
            ...
            FileHandle.standardError.write(
              Data(
                "Warning: could not cache extracted clone prompt: \(error.localizedDescription)\n"
                  .utf8)
            )
          }
        ...
        FileHandle.standardError.write(
          Data(
            "Warning: could not extract clone prompt from .vox: \(error.localizedDescription)\n"
              .utf8)
        )
      }
    }
